...
dat %>% 
  ...
  summarize(first_sampled = min(date),
            ...
            years_sampled = as.double(max(dat$date) - min(dat$date)) / 365.25,
...
lmm_out <- models2 %>% 
  mutate(rate = intervals(mod)$fixed["date", 2] * 365.25,
         CI_low = intervals(mod)$fixed["date", 1] * 365.25,
         CI_high = intervals(mod)$fixed["date", 3] * 365.25) %>% 
  select(-mod)
