...
*This document is intended for use by National Estuarine Research Reserve (NERR) technical staff to calculate, visualize, and describe rates of elevation change at Surface Elevation Table (SET) stations. These products are output from a 2018-2019 NERRS Science Collaborative Catalyst Grant project nicknamed "SETr". For more information on options to customize this report, see the document "Guide to the SETr Workflow" in the main folder of SETr outputs.* 



# General recommendations on analysis of SET data  

Be aware that **linear models are not appropriate for calculating rates of change at all sites. Use discretion when interpreting these results.**  

# come back to this  

+  Before analysis, the analyst should carefully examine plots of the data to ensure that a linear model is appropriate. If the points exhibit curvature or if there is some sort of a changepoint, this type of analysis may not be appropriate.  
...
+  The analyst should employ a statistically valid model fitting strategy. When using LMMs, this should also include addressing issues such as: ensuring convergence of the numerical optimization, exploring sensitivity to starting values in the optimization procedure, determining whether a random slope model or a random slope/random intercept model is most appropriate, and verifying model assumptions.  
+  Determine whether a model that includes temporal dependence is appropriate.
...
We start by reading in the long SET measurement dataset, converted from other formats by earlier scripts. Pin heights have also been converted to mm if they had been recorded in cm.  
...
**Note** that the R code dealing with these flags looks for an EXACT match. e.g., if "LHE" is specified for removal, only values with "LHE" will be removed. "LHE CB" and "CRM LHE" will *not* be removed, and need to be specified separately. To remove a combination of codes, a new line may need to be created in the `qaqc_codes` tab of the input spreadsheet.  
...
**This is an oversimplification of sea level rise:**  the slope is that of long-term, local SLR. This was based on the SLR rate from the NWLON station listed above, and does not include actual water level data. This line also does not account for accelerating sea level rise.  

These graphs are meant to illustrate and give context for SET elevation change relative to long-term SLR.  
...
NAVD88 information varied between reserves in this project; some reserves were not entirely sure what the numbers they had related back to. So **this graph is meant to show changes along a relative elevation gradient only.** We have not overlaid water levels because we could not always be sure of the appropriate offset.  
...
ggplot(cumu_navd88, aes(x = date, y = mean_cumu, color = set_id)) +
  ...
  labs(title = "SET cumulative change",
       subtitle = "each SET's NAVD88 elevation used for its first sample date",
...
Next discuss model fitting… All models were fit in R (reference…) using the nlme package (reference…). This type of approach is similar to the analysis performed in Cahoon 2019.
...
This could take some explaining. Essentially, if the 95% CI doesn't include 0, that's the same as p < 0.05; slope differs significantly from 0. Comparisons to sea level change get more complicated because there's not a single number we're comparing to; there's a number and a confidence interval. And they're generated using different methods than what we've used to calculate the SET rates of change. So the closest we can get to "is this a significant difference" is "do the CIs overlap?" It's not exactly the same as a t-test, and the probability of a type I error is higher than 0.05. It is, however, the general idea of "how confident are we". This same idea plays into everything else below; it's just a matter of how it's represented. So please share your thoughts.   

## Overall Summary Table  

In this table, the SETs are ordered by their rate of change, from lowest at the top to highest at the bottom. The rate and 95% CI (all in mm/yr) for each SET is provided. The last 6 columns show comparisons to: zero (is elevation at the SET changing?), long-term SLR, and 19-year water level change. Each comparison takes up two columns: a comparison of point estimates (is the SET rate higher or lower than the estimated SLR rate), and whether the confidence intervals overlap or not (see above).  
...
rates_comp_table <- rates_comp_table %>% 
  ...
  # align(align = "center", part = "body", j = 5:10) %>% 
  # align(align = "ce", part = "header", i = 2, j = c(5, 7, 9)) %>%
  # align(align = "center", i = 1, part = "header") %>% 
  # autofit()

forced_widths <- c(1, rep(0.7, 9))
width(rates_comp_table, width = forced_widths)
```

```{r}
# thank you to Sarah from stackoverflow for 
# having and solving the problem I'm experiencing
...
FitFlextableToPage(rates_comp_table)
```

```{r}
FitFlextableToPage <- function(ft, pgwidth = 7){

  ft_out <- ft %>% width(width = c(1, rep(0.7, 3), rep(0.85, 6)))
...
# Type I error rate language - come back to  

This method of comparison was chosen because different methods were used to calculate rates for sea level rise (ARIMA) and SET elevation change (LMMs). **Note** that the type I error rate is not actually 5% in these cases. 
