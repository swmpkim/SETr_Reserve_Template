...
# save it
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_noLine.png")
ggsave(out_file, width = 7, height = 7, units = "in")
...
plot_cumu_set(dat_cumu$set, columns = opts_long$multi_cols,
              pointsize = 2.5)
# save it
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_withLine.png")
ggsave(out_file, width = 7, height = 7, units = "in")
```
...
p +
  ...
  geom_abline(data = intercepts,
              ...
              color = "red3",
              size = 1) +
  labs(subtitle = "red line approximates sea level rise; blue line is linear model of SET change")
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_withLineAndSLR.png")
ggsave(out_file, width = 7, height = 7, units = "in")
...
p + 
  ...
  labs(title = "Cumulative Change since first reading",
       subtitle = "red line approximates sea level rise")
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_withSLR.png")
ggsave(out_file, width = 7, height = 7, units = "in")
...
ggplot(dat_cumu$set, aes(x = date, y = mean_cumu)) +
  ...
  geom_abline(data = intercepts,
              ...
              color = "red3",
              size = 1)
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_linesOnly.png")
ggsave(out_file, width = 7, height = 7, units = "in")
```
...
```{r, fig.width = 5.8, fig.height = 8.2}
...
ggplot(cumu_navd88, aes(x = date, y = mean_cumu, color = set_id)) +
  ...
  labs(title = "SET cumulative change",
       ...
       color = "SET")
out_file <- here::here("R_output", "figures", "cumu_change_plots", "cumu_change_NAVD88.png")
...
out_file <- here::here("R_output", "figures", "summary_plots", "summary_plot_veg.png")
ggsave(out_file)
```
...
if(sum(is.na(mdat$numerical_order)) + sum(is.na(mdat$user_friendly_set_name)) == 0){
  ...
  p <- plot_rate_comps(data = lmm_out_ordered, plot_type = 3, 
                       ...
                       rates = rate) 
  out_file <- here::here("R_output", "figures", "summary_plots", "summary_plot_ordered.png")
  ggsave(out_file, p)
  print(p)
  ...
  out_file <- here::here("R_output", "figures", "summary_plots", "summary_plot_ordered_veg.png")
  ggsave(out_file, p)
  print(p)
  ...
  p <- plot_rate_comps(data = lmm_out_ordered, plot_type = 3, 
                       ...
                       set_ci_high = CI_high, slr = slr, slr_ci = slr_ci,
                       rates = rate) 
  out_file <- here::here("R_output", "figures", "summary_plots", "summary_plot_ordered_friendly_names.png")
  ggsave(out_file, p)
  print(p)
  ...
  p <- plot_rate_comps(data = lmm_out_ordered, plot_type = 3, 
                       ...
                       set_ci_high = CI_high, slr = slr, slr_ci = slr_ci,
                       rates = rate, color_by_veg = TRUE, veg = veg) 
  out_file <- here::here("R_output", "figures", "summary_plots", "summary_plot_ordered_friendly_names_veg.png")
  ggsave(out_file, p)
  print(p)
}
```
