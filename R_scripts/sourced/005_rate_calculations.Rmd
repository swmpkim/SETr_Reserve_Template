...
Graphs for each SET individually are not shown here but have been saved:  
...
for(i in seq_along(all_sets)){
  ...
  out_name <- paste("R_output", "figures", "cumu_change_plots", "individual_sets", plot_name, sep = "/")
out_file <- here::here(out_name)

ggsave(out_file, p, width = opts_long$width_short, height = opts_long$height_short, units = "in", scale = 0.8)

cat(paste(out_name, "has been saved \n "))
}
```
...
```{r plot_slr_ci_lmm, fig.width = opts_long$width_short, fig.height = opts_long$height_short, eval = enough_dat}
...
```{r plot_slr_ci_lmm_veg, fig.width = opts_long$width_short, fig.height = opts_long$height_short, eval = enough_dat}
# colored by veg
plot_rate_comps(data = lmm_out, plot_type = 3, set_ids = set_id,
                set_ci_low = CI_low, set_ci_high = CI_high,
                rates = rate, slr = slr, slr_ci = slr_ci,
...
```{r plot_slr_ci_lmm_ordered, fig.width = opts_long$width_short, fig.height = opts_long$height_short, eval = enough_dat}
