...
If the custom start and/or end dates were specified for this analysis, the dataset is subsetted here and the chosen date range will be printed below this paragraph. These options can be changed in the file `metadata/user_defined_inputs.xlsx`.  
...
# cleanup
rm(dat_not_m, m_not_dat, mdat_sub, metadata_setid, data_setid, filelist, path)
```
...
This rate is reported by `r slr_res$nearest_nwlon_station`, NWLON station number `r as.character(slr_res$nwlon_station_number)` based on data from *`r slr_res$yr19_start`* to *`r slr_res$yr19_end`*.  
...
Make a nested function out of this because all my inputs will be the same each time except for plot type and whether to color by veg. Although when putting things in order, it will come from a different data frame, and the set id column name is different, so.... yeah, specify all of them to be safe.    

```{r, fig.width = opts_long$width_short, fig.height = opts_long$height_short}
