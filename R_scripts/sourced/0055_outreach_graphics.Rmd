...
# Feedback - ignore this; it will be worked in eventually  

What I might suggest is having a really short summary on the top, and then maybe a boldface summary sentence or two at the top of each subsection, so that someone could get the take home messages really quickly, but then go in and read the fine print if desired before leading a workshop, etc.  
I also would suggest starting with a zoomed out background paragraph that briefly reviews, in a sentence or two each  
--why we love marshes (key values/services)
--why elevation is critical for marshes (can’t have too much or too little inundation by tidal waters, and this is function of elevation)
--that accelerated SLR poses emerging threat and makes tracking elevation extra critical; need marshes to track SLR
--that there are lots of factors that affect marsh elevation changes.  Some of these are in top meters which: organic deposition by roots below ground, surface deposition of inorganics.  SET is a way of measuring these shallow elevation changes.  There are also deeper elevation changes (due to groundwater overdraft, geologic uplift or subsidence, etc.), and these can be just as important for marsh tracking SLR, but are not covered by SETs.  Ideally you could plot shallow+deep elevation changes relative to SLR (and some of us have both data and can), but here you’re focusing on the shallow, because it’s part of a NERR monitoring network.
...
“On a map” section you refer to figure 11 and 12, did you mean map 1 and 2? (answer: probably; still working on captions)  
