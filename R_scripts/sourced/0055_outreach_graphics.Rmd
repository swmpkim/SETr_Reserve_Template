...
The "Sentinel Site Application 1" (SSAM-1) module of the NERR Sentinel Site Program integrates System-Wide Monitoring Program (SWMP) water level and vegetation data with elevation measurements, for the purpose of assessing the impacts of sea level change. Surface Elevation Table (SET) technology, a means of measuring height of the marsh substrate, is currently an elective but widely implemented component of SSAM-1. In this project, previously acquired NERRS SET data is analyzed to answer the question – are marsh surfaces keeping pace with sea level rise? Note that this is an analysis looking back through time; it is not a forecasting tool.
...
These measurements are repeated over time. Because of the stability of the SET mark, each pin is lowered to the surface in the *same* location, time after time. The measurement can be used to calculate where the surface is, and other measurements can relate it back to standard frameworks, such as local mean sea level. After several years, scientists can calculate a "rate of change" for the marsh surface. Is it getting higher, lower, or not changing? Because marshes are so important to coastal areas - they provide wildlife habitat, nursery habitat for seafood species, recreational areas, and flood protection - we especially want to know if the surface is growing quickly enough to keep up with sea level rise.  
...
```{r, fig.width = 7, fig.height = 6, fig.cap = 'Figure 12. Summary graph of the rate of marsh surface elevation change at each SET, colored by dominant vegetation type.'}
p <- plot_rate_comps(data = rate_summ, plot_type = 1, 
                     set_ids = user_friendly_set_name, set_ci_low = CI_low,
                     set_ci_high = CI_high, 
                     comp1 = slr, 
...
```{r, fig.width = 7, fig.height = 5, fig.cap = 'Figure 13. Summary graph of the rate of marsh surface elevation change at each SET, colored by dominant vegetation type, with whiskers to indicate 95% confidence intervals for the SETs.'}
p <- plot_rate_comps(data = rate_summ, plot_type = 2, 
                     set_ids = user_friendly_set_name, set_ci_low = CI_low,
                     set_ci_high = CI_high, 
                     comp1 = slr, 
...
```{r, fig.width = 7, fig.height = 7, fig.cap = 'Figure 14. Summary graph of the rate of marsh surface elevation change at each SET, colored by vegetation type, with whiskers to indicate 95% confidence intervals for the SETs and shading to represent a 95% confidence interval for sea level change.'}
p <- plot_rate_comps(data = rate_summ, plot_type = 3, 
                     set_ids = user_friendly_set_name, set_ci_low = CI_low,
                     set_ci_high = CI_high, 
                     comp1 = slr, 
